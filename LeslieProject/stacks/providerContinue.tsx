...
const ProviderContinueScreen = ({
  route,
}: {
  route: RouteProp<LoginStackRouteType, "ContinueProvider">;
}) => {
  ...
  const handleInfoSubmit = () => {
    ...
    createUserWithEmailAndPassword(auth, email, password)
      .then(async (res) => {
        const keyId = res["user"]["uid"]
        const userRef = doc(db, "users", keyId)
        const userObj: { [key: string]: unknown } = {
          company,
          firstName,
          lastName,
          DOB: dob,
        ...
        if (imageUri) {
          const storageRef = ref(storage, keyId + ".jpg")
      ...
      ImagePicker.launchImageLibraryAsync({
        ...
        .then((res: any) => {
          if (!res.canceled && res.base64) {
            const base64Data: any = `data:image/jpeg;base64,${res.base64}`
  ...
  return (
    ...
    >
      <Text style={{ ...styles.title, color: theme.textColor, marginTop: 30 }}>
      ...
      <Box style={styles.DOB}>
        <Text style={[styles.DOBtext, { color: theme.textColor }]}>
          Date of birth:
        </Text>
      ...
      {imageUri === null ? (
        <Image
          source={require("../assets/ppPlaceholder.png")}
